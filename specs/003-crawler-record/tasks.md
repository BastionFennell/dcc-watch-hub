...
## Revision 2 additions from the UX review triage (`specs/reviews/2026-09-15-ux-review-triage.md`)

- [ ] T334 Remove party rank everywhere (author: DCC has individual rank only): `rank` event becomes `{ t, actor, rank }` (no `scope`); `normalizeEvent` maps legacy `scope: 'party'` rows to `unknown` and accepts legacy `scope: 'crawler'` by dropping the field; `initialState.partyRank` removed from types/state/schema (tolerated and dropped with a warn if present); `rankSeries(events, t, actorId: string)`; delete `copy.feedText.rankParty`, `copy.partyRankLine`, the feed header party line and `EventFeed.partyRank`; converter `rank` row = field1 rank (legacy form `crawler, rank` accepted with a WARN); samples, fixtures, CSVs, schema, README, and tests updated; v1/v2 spec FR-141 superseded (note in 003 spec).
- [ ] T335 [P] Empty states: feed at zero events (`copy.feedStandby` "Standing by. The System reports when the broadcast begins."); map panel header shows `sectorsRevealed` count and an empty line when zero.
- [ ] T336 [P] Sample data: ep2 `initialState.map.revealed` seeded with ep1's reveals (same floor); README authoring rule "an episode's initial map includes every prior reveal on that floor".
- [ ] T337 [P] Record backdrop opaque from the first frame (fade only the dialog box).
- [ ] T338 [P] Accessible names: real separators between handle/player and class/level in frames, glance, and dossier header (aria-hidden "·" + sr-only ", "); relabel player as "played by {player}" (`copy.playedBy(player)`).
- [ ] T339 [P] Party rail: wrap 3 + 2 below 1100 px when the feed column is present; fixed-height pips row on every frame (no jitter); horizontal scroll strip at ≤ 480 px instead of a ragged 3 + 2.
- [ ] T340 Stage caption row: move the caption out of the player into a slim row above the timeline — left `Ep N · Floor N — {title}` (title visible at last), right `formatTime(t)`; remove `StageCaption` from the stage; update tests and the v1 §5 deviation note in the 003 spec.
- [ ] T341 [P] Timeline: custom hover/focus tooltip (instant, touch-friendly) replacing `title`, a compact legend under the strip (chapter/boss/loot/achievement/level-up/story), and a distinct playhead marker separate from the fill.
- [ ] T342 [P] Feed items click-to-seek with a visible `formatTime(t)` on each row (`onSeek` prop threaded from the page; items become buttons; the pinned sponsor too).
...
- [ ] T344 [P] HP label: keep the sheet-faithful ten-segment strip; add a mono caps "HP" label (`copy.hpLabel`) before the strip wherever `HpSegments` renders (glance, record vitals, stacked dossier), with the numbers after it as today.
