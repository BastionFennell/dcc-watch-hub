...
- [X] T332 [P] `FullRecordDialog`: anchor the dialog to a fixed top offset (e.g. `margin-top: 5vh`, `align-items: flex-start`) so a shrinking seek never re-centres it vertically (US2 scenario 2).
- [X] T333 [P] `RankSparkline`: accept `preserveAspectRatio="none"` (or a `stretch` prop) so the glance row draws full width; keep the dialog's vitals usage as is.
