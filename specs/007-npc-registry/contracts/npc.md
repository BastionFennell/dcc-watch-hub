...
- `RegistryBrowser` gains `t: number` and `currentEvents: readonly AnyEvent[]` (the current episode's events); it builds a clipped episodes map (`current → { ...episode, events: events.filter(e => e.t <= t) }`) over the cached index inputs and recomputes `registryIndex` per render (pure; small). Expose the clipping as a pure helper `clipEpisodeToPlayhead(episode, t)` in `src/engine/registry.ts` and `registryIndexAt(show, registry, episodes, currentEpisodeId, t)`.
